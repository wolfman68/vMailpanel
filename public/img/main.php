...
case "insert_domain" :
	...
	// Add postmaster alias
        $r_m = PhammLdap::phamm_add ('cn=postmaster,vd='.$domain_new.','.LDAP_BASE,$entry);

//        $entry_pm["objectclass"][0] = "top";
//        $entry_pm["objectclass"][1] = "VirtualMailAccount";
//        $entry_pm["cn"] = "Postmaster";
//        $entry_pm["sn"] = "Postmaster";
//        $entry_pm["mail"] = "postmaster@".$domain_new;
//        $entry_pm["userPassword"]  = phamm_password_hash($password1);
//        $entry_pm["maildrop"] = "postmaster@".$domain_new;
//        $entry_pm["accountActive"] = "TRUE";
//        $entry_pm["creationDate"] = date('Ymd');
//        $entry_pm["lastChange"] = time();

        // Ad postmaster mailbox
//        $r_p = PhammLdap::phamm_add ('mail=postmaster@'.$domain_new.',vd='.$domain_new.','.LDAP_BASE,$entry_pm);
        
//        $entry_postmaster["objectclass"][0] = "top";
        $entry_postmaster["objectclass"][0] = "VirtualMailAlias";
        $entry_postmaster["cn"] = "Postmaster";
        ...
        $entry_postmaster["accountActive"] = "TRUE";
        $entry_postmaster["creationDate"] = date('Ymd');
        $entry_postmaster["lastChange"] = time();
	
	// Add postmaster mailbox 
        $r_p = PhammLdap::phamm_add ('mail=postmaster@'.$domain_new.',vd='.$domain_new.','.LDAP_BASE,$entry_postmaster);
        $email = "postmaster@".$domain_new;
        $r_w = mail($email,$welcome_subject,$msg,$headers,'-f postmaster@'.$domain_new);
	...
	// Add abuse alias
        ...
        $entry_spam["objectclass"][0]	= "top";
        $entry_spam["objectclass"][1]	= "VirtualMailAlias";
        $entry_spam["cn"] = "Spam";
        $entry_spam["sn"] = "Spam";
        $entry_spam["mail"] = "spam@".$domain_new;
        $entry_spam["maildrop"] = "postmaster@".$domain_new;
        $entry_spam["accountActive"] = "TRUE";
        $entry_spam["creationDate"] = date('Ymd');
        $entry_spam["lastChange"] = time();
	
	// Add spam alias
        $r_s = PhammLdap::phamm_add ('mail=spam@'.$domain_new.',vd='.$domain_new.','.LDAP_BASE,$entry_spam);

        if ($r_d && $r_m && $r_p && $r_a && $r_s && $_w)        
//        if ($r_d && r_p && r_pm && $r_a && $r_s)
        {

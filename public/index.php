...
if (isset($_GET["exit"]))
{
    ...
    refresh ( 'index.php', $force_meta = false, REFRESH_TIME );
...
elseif($login->login_check())
{
    ...
    if (!$r_bind)
	refresh ( 'index.php?exit=1', $force_meta = false, REFRESH_TIME );
...
//    if (isset($domain) && !empty($domain))
	...
	$ga = explode(';',$command);
        ...
        if ($ga[0] == 'account')
            refresh ('index.php?action=domain_view&amp;domain='.$domain, $force_meta = false, REFRESH_TIME);
        else
            refresh ('index.php', $force_meta = false, REFRESH_TIME);
    }
    else

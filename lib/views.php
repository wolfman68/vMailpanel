...
function login_form()
{
    ...
    $tag .= '<a href="'.ORG_URL.'">'."\n";
    $tag .= "<img src=\"img/logo.jpg\" width=\"120\" alt=\"vMailpanel logo\" /></a>"."\n";
...
function navigation ($initial)
{
    ...
    $tag .= '<li><a href="index.php?domain=NONE&amp;initial=">'._("All domains")."</a></li>"."\n";
    if ($domain)
    {
        $tag .= '<li><a href="index.php?action=domain_view&amp;mail=NONE&amp;domain='.$domain.'&amp;initial=">';
        $tag .= $domain.'</a></li>'."\n";
    }
    if ($mail && $domain)
        $tag .= '<li>'.$mail.'</li>';
...
function action_select($dispaly)
{
    ...
    $tag .= '<option value="">'._("... execute bulk / single command").'</option>'."\n";
